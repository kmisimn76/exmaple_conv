					 ...
					 ) 
{
	...
	do {	// reduction loop for entire C
		...
		float4 f1 = weights_cache[1];
		float4 f2 = weights_cache[2];
		float4 f3 = weights_cache[3];
		weights_cache = filters_loc1;
		float4 f4 = weights_cache[0];
		float4 f5 = weights_cache[1];
		float4 f6 = weights_cache[2];
		float4 f7 = weights_cache[3];
		filters_loc0 += 4;
		filters_loc1 += 4;
#else
	...
	if (X+1 < dst_tensor_shape.x)
	{
		...
		if(iszero != 0.f) {
#ifdef IS_CONV_RA
			int idx = atomic_add(&nnzg_per_hw[(X+1) * dst_tensor_shape.y + (Y+0)], 1);
			//int idx = nnzg_per_hw[(X+1) * dst_tensor_shape.y + (Y+0)]++;
			nzg_val[idx] = Z;
#else
			nzg_val[Z] = 1;
#endif
	...
	if (X+1 < dst_tensor_shape.x && Y+1 < dst_tensor_shape.y)
	{
		...
		if(iszero != 0.f) {
#ifdef IS_CONV_RA
			int idx = atomic_add(&nnzg_per_hw[(X+1) * dst_tensor_shape.y + (Y+1)], 1);
			//int idx = nnzg_per_hw[(X+1) * dst_tensor_shape.y + (Y+1)]++;
			nzg_val[idx] = Z;
#else
			nzg_val[Z] = 1;
#endif
