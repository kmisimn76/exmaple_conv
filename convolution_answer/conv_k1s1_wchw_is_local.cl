...
#define SIZE 16
	...
	int clid = get_local_id(2);
	__local ushort nzg_local[64][SIZE]; //[XY][C]
	__local int nnzg[64]; // [XY]
	if(clid==0)
		nnzg[lid] = 0;
...
//			out_nzg_val[idx] = (Z * M_GROUP + 0);
			int idx = atomic_add(&nnzg[lid], 1) //local atomic
			nzg_local[lid][idx] = (Z*M_GROUP+0);
#else
...
/*	barrier(CLK_LOCAL_MEM_FENCE);
	...
	{
#ifdef IS_CONV_RA
		int idx = atomic_add(&out_nnzg_per_hw[X * dst_tensor_shape.y + Y], nnzg[lid]);
		for(int i=0;i<nnzg[lid];i++) {
			out_nzg_val[i+idx] = nzg_local[lid][i];
		}
#endif
	}*/
	out_nzg_val[nnzg[lid]-1] = Z;
#endif
}
